...
# Example: publish-registry-module-vcs.sh example-org example-org/terraform-aws-MSKProvisioned ot-XCpQWKuCie2gmXtc

set -euo pipefail

if [[ $# -lt 3 ]]; then
  echo "Usage: $0 <organization> <github-repo> <oauth-token-id> [display-identifier]"
  echo "Example: $0 example-org example-org/terraform-aws-MSKProvisioned ot-XCpQWKuCie2gmXtc"
  echo ""
...
if [[ ! "$OAUTH_TOKEN_ID" =~ ^ot- ]]; then
  echo "Error: OAuth token ID should start with 'ot-' (e.g., ot-XCpQWKuCie2gmXtc)"
  exit 1
fi
